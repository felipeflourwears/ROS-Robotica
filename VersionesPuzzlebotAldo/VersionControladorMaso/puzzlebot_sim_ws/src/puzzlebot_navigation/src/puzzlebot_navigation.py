...
class Navigator():
    ...
    def actionCallback(self, goal):
        ...
        while not success:
            ...
            thetaErrorM=thetam-theta_curr
            if thetaErrorM>pi:
                thetaErrorM-=2*pi
            elif thetaErrorM<-pi:
                thetaErrorM+=2*pi
            
            thetaErrorD=theta_goal-theta_curr
            if thetaErrorD>pi:
                thetaErrorD-=2*pi
            elif thetaErrorD<-pi:
                thetaErrorD+=2*pi

            distanciaL=sqrt((y_goal-y_curr)** 2 + (x_goal-x_curr)** 2)
            ...
            if STATE == 0:
                if dt==0: dt=0.00001
                cmd_vel.angular.z=(kP*thetaErrorM)+(kD*((thetaErrorM-errorPrevioM)/dt))
                if cmd_vel.angular.z>=0.80:
                    cmd_vel.angular=0.80
                elif cmd_vel.angular.z <=-0.80:
                    cmd_vel.angular.z=-0.80
                cmd_vel.linear.x=0
                errorPrevioM=thetaErrorM
                if abs(thetaErrorM)<THETA_THRESHOLD:    
                    STATE += 1
            elif STATE == 1:
                cmd_vel.angular.z=(kP*thetaErrorM)+(kD*((thetaErrorM-errorPrevioM)/dt))
                if cmd_vel.angular.z>=0.80:
                ...
                errorPrevioM=thetaErrorM
                if distanciaL<DIST_THRESHOLD:
                    STATE += 1
            elif STATE == 2:
                if dt==0: 
                    dt=0.00001
                cmd_vel.angular.z=(kP*thetaErrorD)+(kD*((thetaErrorD-errorPrevioD)/dt))
                if cmd_vel.angular.z>=0.80:
                    cmd_vel.angular=0.80
                elif cmd_vel.angular.z <=-0.80:
                    cmd_vel.angular.z=-0.80
                cmd_vel.linear.x=0
                errorPrevioD=thetaErrorD
                print("Error previo D",errorPrevioD)
                print("thetaErrorD",thetaErrorD)

                if abs(thetaErrorD)<THETA_THRESHOLD:
                    STATE+=1
            else:
                    cmd_vel.angular.z=0
                    cmd_vel.linear.x=0
