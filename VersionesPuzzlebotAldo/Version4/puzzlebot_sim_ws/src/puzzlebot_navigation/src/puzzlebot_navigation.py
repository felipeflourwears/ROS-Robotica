...
class Navigator():
    ...
    def actionCallback(self, goal):
        ...
        while not success:
            ...
            deltaError=(errorAng-errorPrevio)*dt
            wAngular=kP*errorAng+kD*deltaError
            
            errorPrevio3=theta_goal-theta_curr
            ...
            #Cambio de estados 
            # thersholdAng1=theta_curr-THETA_THRESHOLD
            # thersholdAng2=theta_curr+THETA_THRESHOLD
            distanciaL=sqrt((y_goal-y_curr)** 2 + (x_goal-x_curr)** 2)
            thesholdPos1=0.1

            if STATE == 0:
                cmd_vel.linear.x=0
                cmd_vel.angular.z=wAngular
                if abs(errorAng)<THETA_THRESHOLD:    
                    cmd_vel.linear.x=0
                    cmd_vel.angular.z=0
                    STATE = 1
                    #state='state2'
            elif STATE == 1:
                rospy.loginfo(distanciaL)
                ...
                cmd_vel.linear.x=0
                cmd_vel.angular.z=wAngular
                if abs(errorPrevio3)<THETA_THRESHOLD:
                    cmd_vel.linear.x=0
                    cmd_vel.angular.z=0
                    success=True
